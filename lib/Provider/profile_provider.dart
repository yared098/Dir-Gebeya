...
class ProfileProvider extends ChangeNotifier {
  ...
  Future<void> fetchProfile({bool forceRefresh = false}) async {
    if (!forceRefresh && _userProfile != null) return;
    ...
    final token = await TokenStorage.getToken();
    if (token == null) {
      _error = "Authentication error. Please log in.";
      _isLoading = false;
      notifyListeners();
      return;
    }
    ...
    try {
      final response = await http.get(
        Uri.parse('${ApiConfig.baseUrl}/profile_api'),
        headers: {'Authorization': 'Bearer $token'},
      );
      ...
      if (response.statusCode == 200) {
        ...
        _error = "Failed to load profile. Try again later.";
      }
    } catch (e) {
      _error = "Unable to fetch profile. Check your connection.";
    }
  ...
  Future<bool> logout() async {
    ...
    try {
      final response = await http.post(
        Uri.parse('${ApiConfig.baseUrl}/logout_api'),
        headers: {
          'Authorization': 'Bearer $token',
          'Content-Type': 'application/json',
        },
      ...
      if (response.statusCode == 200) {
        ...
        _error = "Logout failed. Try again.";
      }
    } catch (e) {
      _error = "Logout error. Check your connection.";
    }
  ...
  Future<bool> uploadProfileImage(File imageFile) async {
    ...
    if (fileSize > 2 * 1024 * 1024) {
      _error = "Image must be under 2MB.";
      notifyListeners();
      return false;
    }
  ...
  Future<void> updateUserStatusInfo() async {
    ...
    try {
      bool serviceEnabled = await Geolocator.isLocationServiceEnabled();
      if (!serviceEnabled) {
        _error = 'Enable location services.';
        notifyListeners();
        return;
      }
