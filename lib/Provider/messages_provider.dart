...
import '../utils/token_storage.dart';  // adjust path
import '../config/api_config.dart';   // adjust path
import '../utils/catchMechanisem.dart'; // Your cache utils import
import 'package:connectivity_plus/connectivity_plus.dart';
import 'package:shared_preferences/shared_preferences.dart';
import 'package:dirgebeya/Model/Messages.dart'; // Message model import
...
class MessagesProvider extends ChangeNotifier {
  ...
  Future<void> fetchMessages({
    ...
    if (!isOnline) {
      ...
      if (cached != null) {
        try {
          // cached is dynamic, decode to List<Message>
          List cachedMessages = cached['data'] ?? [];
          _messages = cachedMessages.map((e) => Message.fromJson(e)).toList();
          _page = cached['page'] ?? page;
          _limit = cached['limit'] ?? limit;
          _totalPages = cached['pages'] ?? 1;
          _error = null;
        } catch (e) {
          _error = "Failed to load cached messages";
          _messages = [];
        }
      } else {
        _error = "No internet and no cached messages available";
        _messages = [];
      }
      _isLoading = false;
      notifyListeners();
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data['data'] != null) {
          ...
          // Cache the full response JSON for offline use
  ...
  Future<bool> markMessageAsReceived({
    ...
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        // Update the local message list if needed
        final index = _messages.indexWhere((msg) => msg.id == id);
        if (index != -1) {
          final updatedMsg = _messages[index];
          _messages[index] = Message(
